...
#define CPU_CLOCK               125000000UL     ///< Specifies the CPU Clock in Hz.
...
#define DAP_DEFAULT_SWJ_CLOCK   1000000UL       ///< Default SWD/JTAG clock frequency in Hz.
...
__STATIC_INLINE uint8_t DAP_GetProductFirmwareVersionString (char *str) {
  ...
  strcpy(str, BoardFirmwareVersion);
  len = (uint8_t)(strlen(BoardFirmwareVersion) + 1U);
  return (len);
}
...
__STATIC_INLINE void PORT_SWD_SETUP (void) {
    gpio_init(GPIO_SWDCLK_PIN);
    gpio_init(GPIO_SWDIO_PIN);
    gpio_init(GPIO_nRESET_PIN);

    gpio_set_dir(GPIO_SWDCLK_PIN, GPIO_OUT);
    gpio_set_dir(GPIO_SWDIO_PIN, GPIO_OUT);
    gpio_set_dir(GPIO_nRESET_PIN, GPIO_OUT);

    gpio_put(GPIO_SWDCLK_PIN, 1);
    gpio_put(GPIO_SWDIO_PIN, 1);
    gpio_put(GPIO_nRESET_PIN, 1);
}
...
__STATIC_FORCEINLINE uint32_t PIN_SWDIO_IN      (void) {
  return (gpio_get(GPIO_SWDIO_PIN) != 0) ? 1U : 0U;
}
...
__STATIC_FORCEINLINE void     PIN_SWDIO_OUT     (uint32_t bit) {
  gpio_put(GPIO_SWDIO_PIN, (bit & 1) ? 1 : 0);
}
...
__STATIC_FORCEINLINE void     PIN_SWDIO_OUT_DISABLE (void) {
  gpio_set_dir(GPIO_SWDIO_PIN, GPIO_IN);
}
